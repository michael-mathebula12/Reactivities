...
public class BaseActivityValidator <T,TDto> : AbstractValidator<T> where TDto : BaseActivityDto
{
    public BaseActivityValidator(Func<T, TDto> selector)
    {
        ...
        .NotEmpty().WithMessage("City is requred");
        RuleFor(x => selector(x).Venue)
        .NotEmpty().WithMessage("Venue is requred");
        RuleFor(x => selector(x).Latitude)
        .NotEmpty().WithMessage("Latitude is requred")
        .InclusiveBetween(-90, 90).WithMessage("Latitude must be between -90 and 90");
        RuleFor(x => selector(x).Longitude)
        .NotEmpty().WithMessage("Longitude is requred")
